...
food_type=["french","italian","chinese","indian","pakistani","spanish","mexican","english","dutch","european"]

_restaurants_index=[];

for _i in food_type:
    for _j in reviews_text: 
        _j=str(_j)
        if(re.search(_i,_j)):
            _restaurants_index.append(reviews_text.index(_j))

    food_in_restautants[_i]=_restaurants_index;
    _restaurants_index=[];
